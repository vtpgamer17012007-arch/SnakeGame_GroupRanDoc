    ...
    def _draw(self):
        ...
        for i, rect in enumerate(self.slots):
            ...
            if slot_data and self.img_slot_frame:
                ...
                static_off_x = 5  # Dịch sang phải (+) / trái (-)
                static_off_y = 0  # Dịch xuống (+) / lên (-)
            ...
            if slot_data and rect.collidepoint(mouse_pos):
                ...
                manual_offset_x = 5  # Dương: sang phải | Âm: sang trái
                manual_offset_y = 0  # Dương: xuống dưới | Âm: lên trên
            ...
            if slot_data:
                name_txt = self.font.render(f"{slot_data['key']}", True, (255, 255, 255))
                self.screen.blit(name_txt, (rect.x + 40, rect.y + 12))
                score_txt = self.font_score.render(f"Score: {slot_data['data'].get('score', 0)}", True, (255, 255, 0))
